...
import { addinvestments, getprofile } from '../../config/apiService';
...
function AddInvestmentModal({ onClose, onSuccess }) {
  ...
  const { handlePay, payLoading, processing, countdown } = useWeb3Payment(
    parsed || 0,
    ({ walletAddress, txHash }) => addinvestments(parsed, walletAddress, txHash, 'web3'),
    onCryptoSuccess
  )
  ...
  const handleSystemPay = async () => {
    ...
    try {
      await addinvestments(parsed, undefined, undefined, 'wallet')
      setSuccess(true)
      onSuccess?.()
    } catch (err) {
  ...
  const handleInvest = () => {
    if (!parsed || parsed <= 0) { setError('Please enter a valid amount greater than 0.'); return }
    setError('')
  ...
  const handleClose = () => {
    const pa = panelRef.current?.animate(
      [{ opacity: 1, transform: 'translateY(0) scale(1)' }, { opacity: 0, transform: 'translateY(32px) scale(0.93)' }],
  ...
  return (
    <>
      {createPortal(
        <div ref={backdropRef} style={{ opacity: 0 }}
            ...
            {success ? (
              <div className="flex flex-col items-center text-center pt-6 pb-2">
                ...
                <button type="button" onClick={handleClose}
                  className="w-full py-3.5 rounded-xl text-sm font-bold text-white cursor-pointer border-none bg-linear-to-r from-[#7F25FB] to-[#CB3CFF] hover:shadow-lg hover:shadow-purple-500/30 transition-all duration-200 mb-3">
                  Done
                </button>
                <button type="button" onClick={handleClose}
              ...
              <>
                ...
                <p className="text-sm text-gray-400 mb-6">Deposit USDT to increase your trading capital limit and unlock higher earning potential.</p>
                <label htmlFor="invest-amount" className="block text-[10px] text-gray-400 uppercase tracking-widest font-semibold mb-2">Amount (USDT)</label>
                <div className="relative mb-1">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm font-semibold">$</span>
                  <input id="invest-amount" type="number" min="1" value={amount}
                ...
                <div className="flex items-start gap-3 mb-6 bg-[#0f0f1e]/60 border border-[#2a2a4a] rounded-lg px-4 py-3">
                  ...
                  <p className="text-xs text-gray-400">Investments are reflected immediately. Refer members to unlock a <span className="text-purple-300 font-semibold">2X capital limit</span> multiplier.</p>
                </div>
                <button type="button" onClick={handleInvest} disabled={loading}
                  className="w-full py-3.5 rounded-xl text-sm font-bold text-white cursor-pointer border-none bg-linear-to-r from-[#7F25FB] to-[#CB3CFF] hover:shadow-lg hover:shadow-purple-500/30 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed">
                  {loading
      ...
      {showPayModal && (
        <PaymentMethodModal
          ...
          onSelectCrypto={handleCryptoPay}
