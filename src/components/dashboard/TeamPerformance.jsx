...
function TeamPerformance({ data }) {
  const items = [
    { icon: ICONS.rank,      label: 'CURRENT RANK',     value: data?.currentRank ?? '—' },
    { icon: ICONS.affiliates, label: 'TOTAL AFFILIATES', value: data?.totalDirectReferrals ?? 0 },
    { icon: ICONS.downlines, label: 'TOTAL DOWNLINES',  value: data?.totalDownline ?? 0 },
    { icon: ICONS.swp,       label: 'SWP PURCHASED',    value: `$${(data?.totalSwpPurchased ?? 0).toLocaleString()}` },
    { icon: ICONS.team,      label: 'TEAM SWP VOLUME',  value: `$${(data?.totalTeamSwpVolume ?? 0).toLocaleString()}` },
  ]
    ...
    <div className="mb-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {items.map((item) => (
          <div
            key={item.label}
            className="rounded-xl border border-[#1e1e3a] bg-[#0d0b2e]/60 p-4 text-center hover:border-purple-500/30 transition-colors duration-200"
          >
            <div className="w-9 h-9 rounded-lg bg-[#1a1a3e] text-gray-400 flex items-center justify-center mx-auto mb-2.5">
              {item.icon}
            </div>
