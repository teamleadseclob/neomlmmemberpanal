...
import { getprofile, getreferalhistory } from '../config/apiService';
...
function ReferralHub() {
  ...
  const totalGross = data?.reduce((sum, item) => sum + (item.grossAmount ?? 0), 0) ?? 0;
  const totalCutoff = data?.reduce((sum, item) => sum + (item.cutoffAmount ?? 0), 0) ?? 0;
  const totalNet = data?.reduce((sum, item) => sum + (item.netAmount ?? 0), 0) ?? 0;
  const history = data ?? [];
